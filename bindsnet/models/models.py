...
from ..learning import PostPre, WeightDependentPostPre, WeightDependentDopamin
...
class DiehlAndCook2015_withDopamin(Network):
    # language=rst
    """
    Implements the spiking neural network architecture from `(Diehl & Cook 2015)
    <https://www.frontiersin.org/articles/10.3389/fncom.2015.00099/full>`_.
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.exc = exc
        self.inh = inh
        self.exc_dopamin = exc_dopamin
        self.dopamin_exc = dopamin_exc
        self.dt = dt
        ...
        dop_exc_conn = Connection(
            source=dopamin_layer,
            target=exc_layer,
            w=w,
            update_rule=WeightDependentDopamin,
            nu=nu_dopamin,
            reduction=reduction,
            wmin = 0.0,
            **kwargs
        ...
        self.add_layer(exc_layer, name="Ae")
        self.add_layer(inh_layer, name="Ai")
        self.add_layer(dopamin_layer, name="Dopamin")
        self.add_connection(input_exc_conn, source="X", target="Ae")
        self.add_connection(exc_inh_conn, source="Ae", target="Ai")
        self.add_connection(inh_exc_conn, source="Ai", target="Ae")
        self.add_connection(exc_dop_conn, source="Ae", target="Dopamin")
        self.add_connection(dop_exc_conn, source="Dopamin", target="Ae")